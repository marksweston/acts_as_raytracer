RSpec.describe Plane do
  describe "#intersect" do
    ...
    it "doesn't intersect with a parallel ray" do
      ray = Ray.new(origin: Point.new(0,10, 0), vector: Vector.new(0, 0, 1))
      expect(plane.intersect(ray: ray)).to eq []
    end

    it "doesn't intersect with a coplanar ray" do
      ray = Ray.new(origin: Point.new(0,0, 0), vector: Vector.new(0, 0, 1))
      expect(plane.intersect(ray: ray)).to eq []
    end

    it "intersects from above" do
      ray = Ray.new(origin: Point.new(0,1, 0), vector: Vector.new(0, -1, 1))
